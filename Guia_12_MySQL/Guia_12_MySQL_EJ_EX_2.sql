...
select codigo_oficina, ciudad as 'ciudad donde hay oficinas' from oficina;
...
select nombre, apellido1 as apellido, apellido2 as 'segundo apellido', email 
from empleado 
where codigo_jefe = 7;

/*4. Devuelve el nombre del puesto, nombre, apellidos y email del jefe de la empresa.*/
select puesto, concat(nombre,' ', apellido1,' ', apellido2) as representante, email 
from empleado 
where codigo_jefe is null;

/*5. Devuelve un listado con el nombre, apellidos y puesto de aquellos empleados que no sean
representantes de ventas.*/
select concat(nombre,' ', apellido1,' ', apellido2) as representante, puesto 
from empleado 
...
select estado as 'Estado del pedido'
from pedido
group by estado;
...
select distinct p.codigo_cliente, p.fecha_pago
from pago p
where date_format(p.fecha_pago, '%YEAR') = 2008;
...
select distinct codigo_pedido as 'Nº de pedido',
codigo_cliente 'Nº de cliente',
fecha_esperada 'Fecha estimada de entrega',
...
select distinct codigo_pedido as 'Nº pedido',
codigo_cliente 'Nº cliente',
fecha_esperada 'Fecha estimada de entrega',
fecha_entrega 'fecha de entrega'
from pedido 
...
SELECT distinct c.codigo_cliente, c.nombre_cliente, concat(e.nombre,' ', 
e.apellido1,' ', e.apellido2) as 'Representante', e.puesto, p.forma_pago
FROM cliente c
INNER JOIN empleado e
...
select distinct c.codigo_cliente, c.nombre_cliente, concat(e.nombre,' ', 
e.apellido1,' ', e.apellido2) as 'Representante', e.puesto
from cliente c
...
select distinct c.codigo_cliente, c.nombre_cliente as cliente, 
concat(e.nombre,' ', e.apellido1,' ', e.apellido2) as 'Representante',
e.puesto, o.ciudad, p.forma_pago
from cliente c
inner join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado
...
select distinct c.ciudad, o.linea_direccion1, o.linea_direccion2
from cliente c
inner join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado
inner join oficina o on e.codigo_oficina = o.codigo_oficina
...
select distinct c.nombre_cliente, concat(e.nombre,' ', e.apellido1,' ', e.apellido2) as 'Representante', o.ciudad
from cliente c
inner join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado
inner join oficina o on e.codigo_oficina = o.codigo_oficina
order by o.ciudad;
...
order by pro.gama desc;

/*Consultas multitabla (Composición externa)
Resuelva todas las consultas utilizando las cláusulas LEFT JOIN, RIGHT JOIN, JOIN.

1. Devuelve un listado que muestre solamente los clientes que no pagaron.*/
select c.nombre_cliente, p.fecha_pago, p.total
from cliente c
...
select c.nombre_cliente, p.fecha_pago, pe.fecha_pedido
from cliente c
left join pago p on c.codigo_cliente = p.codigo_cliente
left join pedido pe on c.codigo_cliente = p.codigo_cliente
where p.fecha_pago is null and pe.fecha_pedido is null;
...
inner join producto pr on p.codigo_pedido = pr.codigo_producto;
select * from detalle_pedido order by codigo_producto;
...
right join pedido pe on c.codigo_cliente = pe.codigo_cliente
where pa.id_transaccion is null
and pe.codigo_pedido is not null;
...
select count(*) as 'Nº de empleados'
from empleado;

/*2. ¿Cuántos clientes tiene cada país?*/
select pais, count(pais) as 'Nº clientes'
from cliente
group by pais
order by pais;

/*3. ¿Cuál fue el pago medio en 2009?*/
select round(avg(total)) as 'promedio 2009'
from pago
...
select min(total) as barato, max(total) as caro
from pago;
